...
Vagrant.configure("2") do |config|
  ...
  config.vm.hostname = "vagrant-sidney"

  # Set the correct timezone
  config.vm.provision :shell, :inline => "echo \"Europe/Zurich\" | sudo tee /etc/timezone && dpkg-reconfigure --frontend noninteractive tzdata"

  # Create a static IP
  config.vm.network :private_network, ip: server_ip

  # Use NFS for the shared folder
  config.vm.synced_folder "~/Development/www", "/var/www",
            id: "core",
            :nfs => true,
